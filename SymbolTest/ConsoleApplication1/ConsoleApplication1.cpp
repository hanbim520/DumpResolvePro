...
void ShowUserStreams(PVOID pMiniDump);
...
const ULONG32 cFirstStreamID = 7;
const ULONG32 cSecondStreamID = 7;
...
int _tmain(int argc, TCHAR* argv[])
{
    ...
    if (argc < 2)
    {
        _tprintf(_T("Usage: ReadUserStream <MiniDumpFile>\n"));
        return 0;
    }
...
void ShowUserStreams(PVOID pMiniDump)
{
    ...
    else
    {
        ...
        {
            printf("Stream (id %u): %s \n", StreamID, pStream);
        }
    }

    // Second stream 

    StreamID = cSecondStreamID;
